...
class LoginController extends HomebaseController {
	...
	function index(){
	    if(!empty($_SESSION['family'])){ //已经登录时直接跳到首页
	        redirect(__ROOT__."/user/center/findex");
	    }else{
	        $this->display(":login");
	    }
	}
...
hello;
    ...
    function flogin(){
        ...
        $family_admin = I('post.username','','htmlspecialchars');
        $family_password = I('post.password','','htmlspecialchars');
        if (empty($family_admin)) {
            $this->error("管理账号或密码错误");
        }
        if (empty($family_password)) {
            $this->error("管理账号或密码错误");
        }

        $family_model=M("Users");
        $where['user_login']=$family_admin;
        $where['user_type']=4;
        $where['user_status']=1;
        $result = $family_model->where($where)->find();

        if(!empty($result)){
                if ($result['user_pass_show'] == $family_password) {
                    $_SESSION["family"]=$result;
                    $this->success("登录成功");
                }else{
                    $this->error("管理账号或密码错误");
                }
