	...
	function _initialize() {
		...
		if($ucenter_syn){
			if(!isset($_SESSION["user"])){
				if(!empty($_COOKIE['deerlive_auth'])  && $_COOKIE['deerlive_auth']!="logout"){
					$deerlive_auth=sp_authcode($_COOKIE['deerlive_auth'],"DECODE");
					$deerlive_auth=explode("\t", $deerlive_auth);
					$auth_username=$deerlive_auth[1];
					$users_model=M('Users');
